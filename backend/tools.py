...
def get_gold_market_data():
    ...
    try:
        ...
        signal_line = np.mean([macd_line])  # 简化版signal线
        ...
        result = {
            ...
            "data_delay_minutes": data_delay_minutes,  # 新增：数据延迟分钟数
...
# ========== 缓存控制（5分钟刷新一次，平衡时效性和API调用量） ==========
@lru_cache(maxsize=1)
def _cached_gold_data(_cache_key):
    """带缓存的金价数据（内部函数，不要直接调用）"""
    return get_gold_market_data()


def get_gold_market_data_with_cache():
    """对外暴露的最终方法：5分钟自动刷新缓存"""
...
# 对外默认使用带缓存的版本（可直接替换原有调用）
get_gold_market_data = get_gold_market_data_with_cache